...
function computeTangents( geometry, MikkTSpace, negateSign = true ) {
	...
	_geometry.setAttribute( 'tangent', new THREE.THREE.BufferAttribute( tangents, 4 ) );
...
function interleaveAttributes( attributes ) {
	...
	// calculate the the length and type of the THREE.InterleavedBuffer
	...
	for ( let j = 0, l = attributes.length; j < l; j ++ ) {
		...
		const iba = new THREE.InterleavedBufferAttribute( THREE.InterleavedBuffer, itemSize, offset, attribute.normalized );
		res.push( iba );

		offset += itemSize;

		// Move the data for each attribute into the new THREE.InterleavedBuffer
